...
export default function Programcilar() {
  ...
  const [öneÇıkanProgramcı, setÖneÇıkanProgramcı] = useState(null);
  const [enIyiler, setEnİyiler] = useState(enIyilerListesi);
  ...
  const oneCikaninIsmi = () => {
    ...
    const secilenProgramci = enIyiler.find((item) => {
      return item.id == öneÇıkanProgramcı;
    });
    return secilenProgramci.isim;
  };

  const stil = {
    fontSize: "1.5em",
    marginTop: "0.5em",
    color: öneÇıkanProgramcı == null ? "royalblue" : "gold", // 🤔 kutlarken renk gold'a dönecek
  };
